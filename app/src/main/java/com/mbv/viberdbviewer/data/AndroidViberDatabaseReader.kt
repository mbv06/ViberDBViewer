...
internal class AndroidViberDatabaseReader(
    ...
    fun loadChats(db: SQLiteDatabase): List<ChatSummary> {
        ...
        db.rawQuery(
            ...
            while (cursor.moveToNext()) {
                ...
                val direct = if (conversationType == SELF_CONVERSATION_TYPE) {
                    self
                } else {
                    cursor.contact(4)
                }
                val activeParticipants = cursor.getInt(9)
                val isGroup = conversationType in GROUP_CONVERSATION_TYPES ||
                    (chatName != null && conversationType != SELF_CONVERSATION_TYPE)
                val participantCount = if (isGroup) {
    ...
    fun loadMessages(db: SQLiteDatabase, chatId: Long): List<ChatMessage> {
        ...
        db.rawQuery(
            ...
            while (cursor.moveToNext()) {
                val formatted = formatAndroidMessage(
                    cursor.getInt(3),
                    cursor.stringOrNull(4),
                    cursor.stringOrNull(5),
                    labels,
                )
                val sender = cursor.contact(6)?.displayName(text(R.string.unknown_sender))
